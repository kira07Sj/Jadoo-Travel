...
const Navigation = () => {
  ...
  return (
    <nav className='w-full px-[1rem] md:px-[3rem] flex justify-between items-center mt-[2rem] md:mt-[3rem] '>
        ...
        <div className={`${isOpen ? 'flex flex-col fixed top-[5rem] right-[1rem] ':'hidden'} z-50 bg-white lg:hidden items-end w-full justify-center  text_secondary font-sans font-medium `}>
            {NavLinks.map((items)=>(
        ...
        <h1 onClick={()=>SetIsOpen(false)} src="./bx-menu-right.svg" alt="menu" className={`text-4xl lg:hidden rotate-45 cursor-pointer ${isOpen ? 'flex':'hidden'}`}>+</h1>
